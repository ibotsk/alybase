...
class ChecklistsController extends AppController {
    ...
    public $uses = array('ListOfSpecies', 'LosComment');
    ...
    public function search() {
        if ($this->request->is('post')) {
            ...
            $options = array('conditions' => array(
                    ...
                    'OR' => array('ListOfSpecies.syn_type !=' => 0, 'ListOfSpecies.syn_type' => null), 
                    'ListOfSpecies.genus ILIKE' => $genus_like,
                    'ListOfSpecies.species LIKE' => $species_like
                ),
                'order' => array('ListOfSpecies.ntype_order', 'ListOfSpecies.genus', 'ListOfSpecies.species',
                    'ListOfSpecies.subsp ASC NULLS FIRST', 'ListOfSpecies.var ASC NULLS FIRST', 
                    'ListOfSpecies.subvar ASC NULLS FIRST', 'ListOfSpecies.forma ASC NULLS FIRST', 
                    'ListOfSpecies.authors', 'ListOfSpecies.id'));
            ...
            if (is_array($data['Filter']['types']) && !empty($data['Filter']['types'])) {
                $types = array('OR' => array());
                foreach ($data['Filter']['types'] as $value) {
                    switch ($value) {
                        case 2:
                            $types['OR'][] = array('ListOfSpecies.ntype' => 'A');
                            break;
                        case 3:
                            $types['OR'][] = array('ListOfSpecies.ntype' => 'PA');
                            break;
                        case 4:
                            $types['OR'][] = array('ListOfSpecies.ntype' => 'S');
                            break;
                        case 5:
                            $types['OR'][] = array('ListOfSpecies.ntype' => 'DS');
                            break;
                        case 6:
                            $types['OR'][] = array('ListOfSpecies.ntype' => 'U');
                            break;
                        case 7:
                            $types['OR'][] = array('ListOfSpecies.syn_type' => 1);
                            break;
                        default:
                            break;
                    }
    ...
    public function detail($id) {
        $this->ListOfSpecies->unbindModel(array('hasMany' => array('History', 'LatestRevision', 'Reference')));
        $result = $this->ListOfSpecies->findById($id);
        ...
        $tree = $this->LosComment->generateTreeList(
                array('LosComment.id_list_of_species' => $id), null, null, '_'
        );
        $comments = array();
        foreach ($tree as $idc => $val) {
            $comment = $this->LosComment->find('first', array(
                'fields' => array('LosComment.id', 'LosComment.username', 'LosComment.institution', 'LosComment.annotation', 'LosComment.date_posted'),
                'conditions' => array('LosComment.id' => $idc, 'LosComment.approved' => true)));
            if ($comment) {
    ...
    public function view_pdf() {
        ...
        App::import('Vendor', 'fpdf', array('file' => 'fpdf/html2pdf.php'));
        $pdf = new PDF_HTML();
    ...
    private function _export($request) {
        ...
        $data = $this->ListOfSpecies->find('all', array(
            'conditions' => array('ListOfSpecies.id' => $explIds),
            'order' => array('ListOfSpecies.ntype_order', 'ListOfSpecies.genus', 'ListOfSpecies.species',
                'ListOfSpecies.subsp ASC NULLS FIRST', 'ListOfSpecies.var ASC NULLS FIRST', 
                'ListOfSpecies.subvar ASC NULLS FIRST', 'ListOfSpecies.forma ASC NULLS FIRST', 
                'ListOfSpecies.authors', 'ListOfSpecies.id')));
    ...
    private function _nominalSynonyms($los) {
        if (!$los['ListOfSpecies']['subsp'] && !$los['ListOfSpecies']['var'] && !$los['ListOfSpecies']['subvar'] && !$los['ListOfSpecies']['forma']) { //only genus, species, authors
            ...
            $nominal = $this->ListOfSpecies->find('first', array('conditions' => $conditions));
            if ($nominal) {
    ...
    private function _basionymFor($s) {
        $basfor = $this->ListOfSpecies->find('all', array(
            ...
            'conditions' => array('ListOfSpecies.id_basionym' => $s['id'], 'ListOfSpecies.syn_type' => 3
    ...
    private function _tripleSynonyms($basId, $nameId, $accNameId = null, $superiorId = null) {
        ...
        $conditions = array('OR' => array(
                ...
                array('ListOfSpecies.id_basionym' => $basId, 'ListOfSpecies.id !=' => $nameId, 'ListOfSpecies.syn_type !=' => 0), //mena, ktore maju rovnake basionymum s vynimkou seba sameho
                array('ListOfSpecies.id_basionym' => null, 'ListOfSpecies.syn_type' => 3, 'ListOfSpecies.id_accepted_name' => $nameId), //mena, ktore maju toto meno ako akceptovane
                array('ListOfSpecies.id_basionym' => null, 'ListOfSpecies.syn_type' => 3, 'ListOfSpecies.id_accepted_name' => $accNameId), //mena, ktore maju spolocne akceptovane meno a su k nemu priradene trojito
            ...
            ), 'ListOfSpecies.id !=' => $accNameId, 'ListOfSpecies.id !=' => $nameId);
        if (!$basId) {
            $conditions = array('OR' => array(
                    'ListOfSpecies.id_basionym' => $nameId,
                    array('ListOfSpecies.id_basionym' => null, 'ListOfSpecies.syn_type' => 3, 'ListOfSpecies.id_accepted_name' => $nameId),
                ...
                ), 'ListOfSpecies.id !=' => $accNameId, 'ListOfSpecies.id !=' => $nameId /* , 'ListOfSpecies.ntype !=' => 'A', 'ListOfSpecies.ntype !=' => 'PA' */);
        }
        if ($superiorId) {
        ...
        $tripleSyns = $this->ListOfSpecies->find('all', array(
            ...
            'order' => array('ListOfSpecies.genus', 'ListOfSpecies.species',
                'ListOfSpecies.subsp', 'ListOfSpecies.var', 'ListOfSpecies.subvar',
                'ListOfSpecies.forma', 'ListOfSpecies.authors')
        ));
    ...
    private function _subordinate($species) {
        $c = array(
            array('ListOfSpecies.subsp' => $species, 'ListOfSpecies.var' => null, 'ListOfSpecies.subvar' => null, 'ListOfSpecies.forma' => null),
            array('ListOfSpecies.subsp' => null, 'ListOfSpecies.var' => $species, 'ListOfSpecies.subvar' => null, 'ListOfSpecies.forma' => null),
            array('ListOfSpecies.subsp' => null, 'ListOfSpecies.var' => null, 'ListOfSpecies.subvar' => $species, 'ListOfSpecies.forma' => null),
            array('ListOfSpecies.subsp' => null, 'ListOfSpecies.var' => null, 'ListOfSpecies.subvar' => null, 'ListOfSpecies.forma' => $species)
        );
        return $c;
    }
